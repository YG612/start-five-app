...
- `GAP_P0_06_TEST_SPEC.md`
- `tests/gap-p0-06/gapP006TestKit.ts`
- `tests/gap-p0-06/taskWorkspace.contract.test.tsx`
...
1. `tests/gap-p0-06`（预期：现生产尚无四象限工作台，新增旅程合法失败；任何已有的基础控制应保持通过）；
2. `tsc --noEmit`（预期 0）；
