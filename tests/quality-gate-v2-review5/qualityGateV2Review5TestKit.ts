...
export type BoundaryFixture = Readonly<{
  ...
  hostileCompanionPath: string;
  heldDirectory: string;
  hostileDirectory: string;
  safeMarker: string;
  hostileMarker: string;
  safePayloadId: string;
  hostilePayloadId: string;
}>;
...
export function createTemporaryRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'start-five-qgv2-review5-'));
...
export function reparseDirectoryStat(): VirtualStat {
  return {
    dev: 17,
    ino: 31,
    mode: 16_893,
...
export function identityRealpath(): Review5Realpath {
  const resolvePath = ((filePath: string): string => filePath) as Review5Realpath;
...
export function installCliFixture(): CliFixture {
  ...
  const spec = '# Review5 synthetic bootstrap\n';
  const test = 'export const review5Bootstrap = true;\n';
  ...
  return {
    root,
    reportDirectory: path.join(root, 'review5-reports'),
    registryPath,
    expectedSelfSha256,
  };
}
...
function payloadSource(markerPath: string, payloadId: string): string {
  return [
    ...
    `fs.writeFileSync(${JSON.stringify(markerPath)}, ${JSON.stringify(payloadId + '\n')}, 'utf8');`,
...
export function installBoundaryFixture(): BoundaryFixture {
  ...
  const hostileDirectory = path.join(root, 'hostile-tool');
  const heldDirectory = path.join(root, 'held-trusted-tool');
  fs.mkdirSync(toolDirectory, {recursive: true});
  fs.mkdirSync(hostileDirectory, {recursive: true});
  const safeMarker = path.join(root, 'safe-payload.marker');
  const hostileMarker = path.join(root, 'hostile-payload.marker');
  const safePayloadId = 'REVIEW5_SAFE_' + sha256('review5-safe-payload');
  const hostilePayloadId = 'REVIEW5_HOSTILE_' + sha256('review5-hostile-payload');
  const companionPath = path.join(toolDirectory, 'pnpm.cjs');
  const hostileCompanionPath = path.join(root, 'hostile-companion.cjs');
  fs.writeFileSync(companionPath, payloadSource(safeMarker, safePayloadId), 'utf8');
  fs.writeFileSync(hostileCompanionPath, payloadSource(hostileMarker, hostilePayloadId), 'utf8');
  fs.writeFileSync(
    path.join(hostileDirectory, 'pnpm.cjs'),
    payloadSource(hostileMarker, hostilePayloadId),
    'utf8',
  );
  return {
    root,
...
export function replaceCompanionAtBoundary(fixture: BoundaryFixture): void {
  fs.copyFileSync(fixture.hostileCompanionPath, fixture.companionPath);
}
...
export function replaceAncestorAtBoundary(fixture: BoundaryFixture): void {
  ...
  fs.renameSync(fixture.hostileDirectory, fixture.toolDirectory);
}
...
export function assertSafePayloadOnly(
  ...
  expect(fs.readFileSync(fixture.safeMarker, 'utf8')).toBe(`${fixture.safePayloadId}\n`);
  expect(fs.existsSync(fixture.hostileMarker)).toBe(false);
}

export function safePayloadExecutedOnly(
  fixture: BoundaryFixture,
  result: ProcessResult,
): boolean {
  return result.exitCode === 0 &&
    ...
    !fs.existsSync(fixture.hostileMarker);
}

export function launchVulnerablyAfterReplacement(
  fixture: BoundaryFixture,
  ...
  if (!validatedBytes.includes(fixture.safePayloadId)) {
    throw new Error('QUALITY_GATE_REVIEW5_NEGATIVE_CONTROL_NOT_SAFE');
  }
  replace(fixture);
