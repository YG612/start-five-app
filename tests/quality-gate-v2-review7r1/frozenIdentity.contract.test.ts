...
describe('QUALITY-GATE-V2 Review7 frozen identities and authoring boundary', () => {
  ...
  it('keeps production, accepted locks, package, and registry at the authoring baseline', () => {
    ...
    const productionStat = fs.lstatSync(productionPath);
    expect(productionStat.isFile()).toBe(true);
    expect(productionStat.isSymbolicLink()).toBe(false);
    expect(sha256(readProject('scripts/quality-gate-v2/index.cjs'))).toMatch(
      /^[0-9a-f]{64}$/,
    );
