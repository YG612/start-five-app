...
import {projectRoot} from './qualityGateV2Review6TestKit';

const REVIEW5_SELF =
  '51b7310aa27023946ca4419f5ed63acbeb81daa6c6c2d18a5ff335368cbcdecd';
const REVIEW5_STATUS = 'REVIEW FAILED / NEVER ACCEPTED';
const REVIEW4_SELF =
...
describe('QUALITY-GATE-V2 Review6 frozen history identities', () => {
  it('preserves all seven Review5 assets and records REVIEW FAILED / NEVER ACCEPTED', () => {
    ...
    expect(REVIEW5_STATUS).toBe('REVIEW FAILED / NEVER ACCEPTED');
    expect(readProject('QUALITY_GATE_V2_REVIEW6_LOCK_CHANGELOG.md'))
      .toContain(REVIEW5_STATUS);
  });

  it('preserves the independently qualified Review4 candidate and all 12 assets', () => {
    const listed = verifyManifest(
  ...
  it('preserves the accepted Review2 PATH normalization generation', () => {
  ...
  it('does not overwrite either canonical generated report during test authoring', () => {
