...
const FINGERPRINT_PROGRESS_RULE: ReminderRule = {
  id: 'progress-before-due',
  kind: 'progress',
  anchor: 'due',
  offsetMinutes: -60,
  progressBelow: 0.5,
...
function fingerprintInput(
  ...
  return reconcileInput(
    ...
    'reminder-field-fingerprint',
    {
      progressRatio: 0.2,
      rules: [START_RULE, FINGERPRINT_PROGRESS_RULE],
      ...overrides,
    },
  );
}

const REMINDER_FINGERPRINT_CHANGES: readonly Readonly<{
  label: string;
  change(input: ReminderReconcileInput): ReminderReconcileInput;
}>[] = [
  {
    label: 'permission',
    change: input => ({...input, permission: 'denied'}),
  },
  {
    label: 'rule id',
    change: input => ({
      ...input,
      rules: [{...START_RULE, id: 'changed-start-id'}, FINGERPRINT_PROGRESS_RULE],
    }),
  },
  {
    label: 'rule kind',
    change: input => ({
      ...input,
      rules: [{...START_RULE, kind: 'rescue'}, FINGERPRINT_PROGRESS_RULE],
    }),
  },
  {
    label: 'rule anchor',
    change: input => ({
      ...input,
      rules: [{...START_RULE, anchor: 'due'}, FINGERPRINT_PROGRESS_RULE],
    }),
  },
  {
    label: 'rule offset',
    change: input => ({
      ...input,
      rules: [{...START_RULE, offsetMinutes: 5}, FINGERPRINT_PROGRESS_RULE],
    }),
  },
  {
    label: 'rule progress threshold',
    change: input => ({
      ...input,
      rules: [START_RULE, {...FINGERPRINT_PROGRESS_RULE, progressBelow: 0.1}],
    }),
  },
  {
    label: 'progress ratio',
    change: input => ({...input, progressRatio: 0.8}),
  },
  {
    label: 'scheduled-start anchor instant',
    change: input => ({
      ...input,
      task: {
        ...input.task,
  ...
  {
    label: 'due anchor instant',
    change: input => ({
      ...input,
      task: {...input.task, dueAt: '2026-08-05T15:00:00.000Z'},
    }),
  },
  {
    label: 'terminal task state',
    change: input => ({
      ...input,
      task: makeCompletedTask(input.task.id, {
        startAt: input.task.startAt,
        ...(input.task.scheduledStartAt === undefined
          ? {}
          : {scheduledStartAt: input.task.scheduledStartAt}),
...
describe('GAP-P0-03A reminder coordination, recovery, and persistence', () => {
  ...
  it.each(REMINDER_FINGERPRINT_CHANGES)(
    'rejects same-operation reuse when normalized reminder semantics change only $label',
    async ({change}) => {
      const first = createHarness();
      const baseline = fingerprintInput();
      const firstResult = await first.service.reconcile(baseline);
      const repositoryRaw = first.reminderBackend.raw;
      const platformRaw = first.schedulerBackend.raw;
      if (repositoryRaw === null || platformRaw === null) {
        throw new Error('EXPECTED_REMINDER_FINGERPRINT_BYTES');
      }

      const reminderBackend = new PhysicalReminderBackend();
      reminderBackend.raw = `${repositoryRaw}`;
      const schedulerBackend = new PhysicalSchedulerBackend();
      schedulerBackend.raw = `${platformRaw}`;
      const restarted = createHarness({reminderBackend, schedulerBackend});
      await expectCode(
        () => restarted.service.reconcile(change(fingerprintInput())),
        'REMINDER_OPERATION_CONFLICT',
      );
      expect(reminderBackend.raw).toBe(repositoryRaw);
      expect(reminderBackend.transactionAttemptCount).toBe(1);
      expect(reminderBackend.commitCount).toBe(0);
      expect(schedulerBackend.queryCount).toBe(0);
      expect(schedulerBackend.calls).toEqual([]);
      expect(schedulerBackend.raw).toBe(platformRaw);
      expect(await restarted.service.getState(firstResult.taskId)).toEqual(
        firstResult,
      );
    },
  );

  it('replays the same operation for non-semantic text, timezone, early-now, and rule-order changes', async () => {
    const first = createHarness();
    const baseline = fingerprintInput();
    const firstResult = await first.service.reconcile(baseline);
    const repositoryRaw = first.reminderBackend.raw;
    const platformRaw = first.schedulerBackend.raw;
    if (repositoryRaw === null || platformRaw === null) {
      throw new Error('EXPECTED_REMINDER_NORMALIZED_EQUIVALENCE_BYTES');
    }
    const reminderBackend = new PhysicalReminderBackend();
    reminderBackend.raw = `${repositoryRaw}`;
    const schedulerBackend = new PhysicalSchedulerBackend();
    ...
    const replay = await restarted.service.reconcile(
      fingerprintInput({
        task: {
          ...baseline.task,
          title: 'Non-semantic title change',
          description: 'Non-semantic description change',
          firstStep: 'Non-semantic first-step change',
        },
        now: '2026-08-05T08:00:00.000Z',
        timeZone: 'America/New_York',
        rules: [FINGERPRINT_PROGRESS_RULE, START_RULE],
      }),
    );
  ...
  it('restores a missing platform schedule from repository bytes and returns detached state', async () => {
    ...
    const callerClone = {
      ...replay,
      intents: replay.intents.map(intent => ({...intent})),
    };
    ...
    expect(replay.intents[0]).toEqual(EXPECTED_START_SNAPSHOT.intents[0]);
