...
describe('GAP-P0-05R2 real AppRoot focus resilience', () => {
  ...
  it('rejects a settled no-focus completion action after public focus start and permits it after durable interruption', async () => {
    const backend = new R2IntegrationBackend();
    ...
    await seedRunningFocus(backend, {
      sessionId: 'gap-p0-05r2r1-active-focus',
    });
    ...
    try {
      await flushUi(240);
      const taskBytesWhileRunning = backend.raw(TASK_STORAGE_KEY);
      const whileRunning = {
        restoreRead: backend.getCalls.includes(FOCUS_SESSION_STORAGE_KEY),
        runningVisible: screen.queryByText(RUNNING_TEXT) !== null,
        completionRendered:
          screen.queryByRole('button', {name: FINISH_TASK_BUTTON}) !== null,
        restoreErrorVisible:
          screen.queryByRole('button', {name: RETRY_RESTORE_BUTTON}) !== null,
        taskStatus:
          (await composition.service.getState()).tasks[0]?.status ?? null,
        taskBytesUnchanged:
          backend.raw(TASK_STORAGE_KEY) === taskBytesWhileRunning,
      ...
      const focusBytesAfterInterrupt = backend.raw(
        FOCUS_SESSION_STORAGE_KEY,
      );
      ...
      await flushUi(240);
      const restart = backend.byteRestartR2();
      const restartComposition = createStartFiveApp({
        storageBackend: restart,
        now: clock.now,
        idGenerator: new SequenceIds(['restart-must-not-consume-id']).next,
      });
      const finalState = await restartComposition.service.getState();
      const final = {
        taskStatus: finalState.tasks[0]?.status ?? null,
        focusBytesUnchanged:
          backend.raw(FOCUS_SESSION_STORAGE_KEY) === focusBytesAfterInterrupt,
        focusHistory: (await readTaskFocusHistory(restart)).map(session => ({
          id: session.id,
          taskId: session.taskId,
          status: session.status,
        })),
