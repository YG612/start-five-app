...
describe('GAP-P0-06R2 workspace projection and selected-task integrity', () => {
  it('keeps durable create single-flight through a failing projection refresh and restarts with exactly one task', async () => {
    ...
    try {
      ...
      const bytesAfterDurableCreate = backend.stableByteSnapshot();

      const retryRefresh = screen.getByRole('button', {
        name: '重试刷新工作台',
      });
      await fireEvent.press(retryRefresh);
      await waitFor(() =>
        expect(screen.getByRole('button', {name: CARD.Q2(title)})).toBeTruthy(),
      );
      expect(backend.stableByteSnapshot()).toBe(bytesAfterDurableCreate);
  ...
  it('removes completed selected A immediately on return and recomputes the workspace recommendation to fallback B', async () => {
    ...
    const query = await restarted.lifecycle.getQueryResult();
    const completedA = query.tasks.find(task => task.id === taskA.id);
    const pendingB = query.tasks.find(task => task.id === taskB.id);
    expect(completedA?.status).toBe('completed');
    expect(pendingB?.status).toBe('pending');
    for (const bucket of query.quadrants) {
      expect(bucket.allTasks.map(task => task.id)).not.toContain(taskA.id);
    }
    const q2 = query.quadrants.find(bucket => bucket.quadrant === 'Q2');
    expect(q2?.allTasks.map(task => task.id)).toContain(taskB.id);
