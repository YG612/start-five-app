...
export class RecordingRunner implements ProcessRunner {
  ...
  async run(request: ProcessRequest): Promise<ProcessResult> {
    ...
    return {
      exitCode: 0,
      signal: null,
      stdout: 'REVIEW2_RECORDING_RUNNER_OK',
      stderr: '',
      timedOut: false,
      timeoutSource: null,
      durationMs: 1,
...
export function createTempDirectory(): string {
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), 'start-five-qgv2-review2-'),
  );
...
export function cliOverrides(
  ...
  return {
    ...
    overrides: {
      ...
      runId: 'qgv2-review2-unit-run',
      processRunner: runner,
    },
  };
}

export async function runRealChild(options: Readonly<{
  args: readonly string[];
  cwd: string;
  environment: Readonly<Record<string, string | undefined>>;
  watchdogMs?: number;
  ...
  const result = childProcess.spawnSync(process.execPath, options.args, {
    cwd: options.cwd,
    env: options.environment,
    windowsHide: true,
    stdio: ['ignore', 'pipe', 'pipe'],
    timeout: options.watchdogMs ?? 20_000,
  });
...
export function readJsonRecord(filePath: string): object {
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error('QUALITY_GATE_V2_REVIEW2_JSON_RECORD_REQUIRED');
  }
  return parsed;
}
