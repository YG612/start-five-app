...
/**
 * Review2's minimum physical-store contract.  `scope` is a stable diagnostic
 * name, never an object-identity lock.  The physical backend, not a repository
 * facade, linearizes compare/exchange across every wrapper and process that
 * publishes the same scope.
 */
...
export interface AtomicReviewBackend extends AsyncKeyValueBackend {
  readonly startFiveAtomic: StartFiveAtomicCapabilityV1;
}

export type Review2Runtime = {
  readonly repository: TaskRepository;
  readonly service: TaskLifecycleService;
};

export function createReview2Runtime(
...
type AtomicPhysicalState = {
  readonly scope: string;
  readonly values: Map<string, string>;
};

export class AtomicPhysicalStore {
  private readonly state: AtomicPhysicalState;

  constructor(scope = 'start-five-review2-physical-scope') {
    this.state = {scope, values: new Map()};
  }

  wrapper(): AtomicBackendWrapper {
    return new AtomicBackendWrapper(this.state);
  }
...
export class AtomicBackendWrapper implements AtomicReviewBackend {
  readonly reads: string[] = [];
  readonly sets: Array<{readonly key: string; readonly value: string}> = [];
  readonly removes: string[] = [];
  readonly compareExchanges: Array<{
    readonly key: string;
  ...
  private nextSetBarrier: Review2Barrier | null = null;
  private signalNextActivity: (() => void) | null = null;

  constructor(private readonly state: AtomicPhysicalState) {
  ...
  private noteActivity(): void {
    const signal = this.signalNextActivity;
    this.signalNextActivity = null;
    signal?.();
  }

  nextActivity(): Promise<void> {
    if (this.signalNextActivity !== null) {
      throw new Error('A2_REVIEW2_ACTIVITY_ALREADY_ARMED');
    }
    return new Promise<void>(resolve => {
      this.signalNextActivity = resolve;
    });
  }

  blockNextCommittedSet(): Review2Barrier {
    if (this.nextSetBarrier !== null) {
      throw new Error('A2_REVIEW2_SET_BARRIER_ALREADY_ARMED');
    }
    const barrier = new Review2Barrier();
    this.nextSetBarrier = barrier;
    return barrier;
  }
...
export class InvalidDeclaredAtomicBackend implements AsyncKeyValueBackend {
  readonly values = new Map<string, string>();
  readonly sets: Array<{readonly key: string; readonly value: string}> = [];
  readonly removes: string[] = [];

  readonly startFiveAtomic = {
    version: 1,
    scope: 'declared-shared-without-compare-exchange',
  };
...
export function parseJsonRecord(raw: string): JsonRecord {
  ...
  if (!isJsonRecord(parsed)) {
    throw new Error('A2_REVIEW2_EXPECTED_JSON_RECORD');
  }
  return parsed;
}
...
function requiredSafeInteger(record: JsonRecord, field: string): number {
  ...
  if (typeof value !== 'number' || !Number.isSafeInteger(value)) {
    throw new Error(`A2_REVIEW2_EXPECTED_SAFE_INTEGER:${field}`);
  }
  return value;
}
...
export function locateLedger(
  ...
  for (const [key, raw] of entries) {
    ...
    if (
      ...
      if (header !== null) {
        throw new Error('A2_REVIEW2_MULTIPLE_LEDGER_HEADERS');
      }
  ...
  if (header === null || pages.length === 0) {
    throw new Error('A2_REVIEW2_LEDGER_NOT_FOUND');
  }
...
export function locateJournal(
  ...
  throw new Error('A2_REVIEW2_JOURNAL_NOT_FOUND');
}
...
export function locateScalableTasks(
  ...
  if (header === null || pages.length < 2) {
    throw new Error('A2_REVIEW2_SCALABLE_TASK_RECORDS_NOT_FOUND');
  }
  pages.sort((left, right) => left.page - right.page);
  return {header, pages};
}
...
function utf8Bytes(value: string): number[] {
  ...
  for (const symbol of value) {
    ...
    if (codePoint === undefined) {
      throw new Error('A2_REVIEW2_UTF8_CODE_POINT_MISSING');
    }
    if (codePoint <= 0x7f) {
...
export function expectedJournalDigest(journal: JsonRecord): string {
  ...
  if (typeof version !== 'number' || !Number.isSafeInteger(version)) {
    throw new Error('A2_REVIEW2_JOURNAL_VERSION_INVALID');
  }
...
export function expectStrongDigest(value: unknown): asserts value is string {
  ...
  if (typeof value !== 'string') {
    throw new Error('A2_REVIEW2_DIGEST_NOT_STRING');
  }
...
export function replaceEntry(
  ...
  if (!replaced) {
    throw new Error(`A2_REVIEW2_RAW_KEY_NOT_FOUND:${key}`);
  }
  return result;
}
...
export function requiredString(record: JsonRecord, field: string): string {
  const value = record[field];
  if (typeof value !== 'string') {
    throw new Error(`A2_REVIEW2_EXPECTED_STRING:${field}`);
  }
  return value;
}
...
export function requiredArray(record: JsonRecord, field: string): unknown[] {
  ...
  if (!Array.isArray(value)) {
    throw new Error(`A2_REVIEW2_EXPECTED_ARRAY:${field}`);
  }
  return value;
}
...
export function mutableRecord(value: unknown): JsonRecord {
  if (!isJsonRecord(value)) {
    throw new Error('A2_REVIEW2_EXPECTED_MUTABLE_RECORD');
  }
  return value;
}

export function expectNoSecretInError(error: unknown, secrets: readonly string[]): void {
