...
describe('GAP-P0-01A2 Review3 global ledger uniqueness', () => {
  it('rejects a non-requested duplicate operationId in one page even under a valid strong digest', async () => {
    ...
    if (page === undefined) {
      throw new Error('A2_REVIEW3_LEDGER_PAGE_MISSING');
    }
  ...
  it('rejects a non-requested duplicate operationId across two pages under a valid strong digest', async () => {
    ...
    if (firstPage === undefined || secondPage === undefined) {
      throw new Error('A2_REVIEW3_TWO_LEDGER_PAGES_REQUIRED');
    }
...
describe('GAP-P0-01A2 Review3 fixed-size private metadata', () => {
  it('uses fixed-size strong task bindings and fingerprints without private task text', async () => {
    const secrets = {
    ...
    const longRuntime = createReview3Runtime(longBackend, {
      now: new CountingClock().now,
    ...
    const shortRuntime = createReview3Runtime(shortBackend, {
      now: new CountingClock().now,
  ...
  it('does not leak private request text through conflict errors, keys, or mutation histories', async () => {
    const backend = new PersistentReviewBackend();
