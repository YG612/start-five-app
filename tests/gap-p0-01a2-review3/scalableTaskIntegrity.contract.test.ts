...
async function makeScalableBackend(): Promise<PersistentReviewBackend> {
  ...
  const runtime = createReview3Runtime(backend, {
    now: new CountingClock('2026-08-06T07:00:00.000Z').now,
...
async function expectTaskCorruptWithoutMutation(
  ...
  const runtime = createReview3Runtime(backend, {
    now: () => {
      throw new Error('A2_REVIEW3_FORBIDDEN_TASK_CORRUPTION_CLOCK');
    },
    idGenerator: () => {
      throw new Error('A2_REVIEW3_FORBIDDEN_TASK_CORRUPTION_ID');
    },
  });
...
function changedSameLengthTitle(task: Record<string, unknown>): void {
  ...
  if (title.length === 0) {
    throw new Error('A2_REVIEW3_EMPTY_TASK_TITLE');
  }
...
describe('GAP-P0-01A2 Review3 scalable task integrity', () => {
  ...
  it('rejects semantically legal task reordering within a page', async () => {
    ...
    if (page === undefined) {
      throw new Error('A2_REVIEW3_SCALABLE_PAGE_MISSING');
    }
    const pageRecord = parseJsonRecord(page.raw);
    ...
    if (first === undefined || second === undefined) {
      throw new Error('A2_REVIEW3_TWO_SCALABLE_TASKS_REQUIRED');
    }
    tasks[0] = second;
    tasks[1] = first;
  ...
  it('rejects a legal page-content swap when page numbers and total count remain valid', async () => {
    const backend = await makeScalableBackend();
    const original = backend.rawSnapshot();
    const scalable = locateScalableTasks(original);
    ...
    if (firstPage === undefined || secondPage === undefined) {
      throw new Error('A2_REVIEW3_TWO_SCALABLE_PAGES_REQUIRED');
    }
  ...
  it('rejects a fixed-length header digest edit without repairing source bytes', async () => {
    const backend = await makeScalableBackend();
    const original = backend.rawSnapshot();
    const scalable = locateScalableTasks(original);
