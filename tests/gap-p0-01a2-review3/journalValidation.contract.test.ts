...
async function capturePreparedUpdateJournal(
  ...
  const runtime = createReview3Runtime(backend, {
    now: new CountingClock('2026-08-06T08:00:00.000Z').now,
...
function parseChangeList(value: string): Array<[string, string | null]> {
  ...
  if (!Array.isArray(parsed)) {
    throw new Error('A2_REVIEW3_JOURNAL_CHANGE_LIST_INVALID');
  }
  ...
  for (const entry of parsed) {
    if (
      ...
      throw new Error('A2_REVIEW3_JOURNAL_CHANGE_ENTRY_INVALID');
    }
...
describe('GAP-P0-01A2 Review3 journal trust boundary', () => {
  ...
  it('preflights intended page bytes against their header digest before any recovery write', async () => {
    const captured = await capturePreparedUpdateJournal();
    const located = locateJournal(captured.capturedEntries);
    expectStrongDigest(located.value.journalDigest);
    const journal = parseJsonRecord(located.raw);
    ...
    if (!changed) {
      throw new Error('A2_REVIEW3_JOURNAL_LEDGER_PAGE_CHANGE_NOT_FOUND');
    }
    journal.afterJson = JSON.stringify(after);
    const forged = replaceEntry(
      captured.capturedEntries,
  ...
  it('compensates an after-write recovery fault to prepared bytes and remains retryable', async () => {
    const captured = await capturePreparedUpdateJournal();
    const located = locateJournal(captured.capturedEntries);
    expectStrongDigest(located.value.journalDigest);
    const backend = PersistentReviewBackend.fromSerializedRawBytes(
    ...
    const cause = new ReviewBackendFault('A2_REVIEW3_RECOVERY_AFTER_WRITE_FAULT');
    ...
    if (typeof error !== 'object' || error === null || !('cause' in error)) {
      throw new Error('A2_REVIEW3_RECOVERY_ERROR_CAUSE_MISSING');
    }
