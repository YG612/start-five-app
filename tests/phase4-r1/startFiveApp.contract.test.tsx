...
function createClockAndIds(): {
  ...
  return {
    ...
    idGenerator: () => {
      sequence += 1;
      return `phase4-r1-generated-${sequence}`;
    },
  };
}

describe('P4-BOOT R1 public composition and hydration contract', () => {
  it('hydrates a durable guest task through the composed root', async () => {
    ...
    backend.seed(PHASE4_STORAGE_KEY, serializePhase4Envelope([hydratedTask]));
  ...
  it('uses the exact exposed service and shared durable state for every root render', async () => {
    const {createStartFiveApp} = loadStartFiveAppModule();
    const backend = new InspectableAsyncKeyValueBackend();
    const sharedTask = makePhase4Task('shared-root-state', {
      title: 'Shared Phase 4 root state',
    });
    backend.seed(PHASE4_STORAGE_KEY, serializePhase4Envelope([sharedTask]));
    const composition = createStartFiveApp({
      storageBackend: backend,
      ...createClockAndIds(),
    });
    await expect(composition.repository.list()).resolves.toEqual([sharedTask]);
    ...
    await waitFor(() => {
      ...
      expect(firstRoot.getByText(/Shared Phase 4 root state/)).toBeTruthy();
    });
    await expect(
      composition.repository.getById(sharedTask.id),
    ).resolves.toEqual(sharedTask);
    ...
    await waitFor(() => {
      expect(getState).toHaveBeenCalled();
      expect(secondRoot.getByText(/Shared Phase 4 root state/)).toBeTruthy();
    });
    await expect(composition.service.getState()).resolves.toMatchObject({
      tasks: [sharedTask],
    });
  ...
  it('keeps service mutations, the exposed repository, the root, and a later app instance on one durable state', async () => {
    ...
    const created = await firstComposition.service.createTask(
      ...
      {operationId: 'phase4-r1:create:shared-state'},
    );
    await expect(
      firstComposition.repository.getById(created.id),
  ...
  it('forwards the injected clock and ID generator into actual service mutations', async () => {
    ...
    const created = await composition.service.createTask(
      ...
      {operationId: 'phase4-r1:create:injected-dependencies'},
    );
  ...
  it('boots to hydration-ready UI locally without invoking network adapters', async () => {
    const {createStartFiveApp} = loadStartFiveAppModule();
    const backend = new InspectableAsyncKeyValueBackend();
    const network = {
    ...
    try {
      ...
      const screen = await render(React.createElement(composition.AppRoot));
      await waitFor(() =>
        expect(screen.getByText(/还没有活动任务/)).toBeTruthy(),
      );
  ...
  it('surfaces corrupt hydration through the exact service and UI without inventing a task', async () => {
    const {createStartFiveApp} = loadStartFiveAppModule();
    const backend = new InspectableAsyncKeyValueBackend();
    ...
    await waitFor(() => {
      expect(getState).toHaveBeenCalled();
      expect(screen.getAllByText('TASK_SNAPSHOT_CORRUPT').length).toBeGreaterThan(
        0,
      );
    });
  ...
  it('keeps the composition and persistence modules independent from the bookkeeping project', () => {
