...
describe('GAP-P0-03A delay diagnosis eligibility, persistence, and privacy', () => {
  ...
  it('trims accepted Unicode text and rejects over-limit text before lookup even after a valid reason', async () => {
    ...
    const error = await expectCode(
      () =>
        rejected.service.submit(
          submitInput({reasonKey: 'task_too_large', privateText: secret}),
    ...
    expect(rejected.backend.readCount).toBe(0);
    expect(rejected.backend.readOnlyOperationLookupCount).toBe(0);
    expect(rejected.backend.transactionAttemptCount).toBe(0);
    expect(rejected.backend.commitCount).toBe(0);
    expect(rejected.backend.events).toEqual([]);
    expect(rejected.clock.calls).toBe(0);
    expect(rejected.ids.calls).toBe(0);
  });
  ...
  it('persists detached suggestions without automatically changing Task or FocusSession bytes', async () => {
    ...
    const sourceFirstStep = {
      kind: 'first_step' as const,
      value: 'Open the document',
    };
    const result = await harness.service.submit(
      submitInput({
    ...
    expect(firstSuggestion).toEqual({
      kind: 'first_step',
      value: 'Open the document',
    });
    sourceFirstStep.value = 'CALLER_SOURCE_MUTATION';
    expect(result.suggestions[0]).toEqual({
      kind: 'first_step',
      value: 'Open the document',
    });
    const stored = await harness.service.listForTask(task.id);
    expect(stored[0]?.suggestions[0]).toEqual({
      kind: 'first_step',
      value: 'Open the document',
    });
  ...
  it('rehydrates from bytes, sorts three distinct timestamps descending and ties by ID, and returns deeply detached queries', async () => {
    ...
    const callerClone = {...firstRecord, reasonKey: 'CALLER_CLONE_MUTATION'};
    expect(callerClone.reasonKey).toBe('CALLER_CLONE_MUTATION');
    expect(firstRecord.reasonKey).toBe('afraid_of_poor_quality');
    const secondList = await restarted.service.listForTask('diagnosis-task');
    expect(secondList[0]?.reasonKey).toBe('afraid_of_poor_quality');
